...
class Admin extends CI_Controller
{
    ...
    public function master_pegawai()
    {
        ...
        abs_pegawai.tanggal_kontrak_akhir,
    ...
    ');
        ...
        $this->db->join('abs_rekening_bank', 'abs_pegawai.nama_bank_id = abs_rekening_bank.id', 'left');

        // Yang Nonaktif ditaruh bawah
        $this->db->order_by("(kode_user = 'Nonaktif')", "ASC");
...
(
    ...
    ');
        ...
        $this->db->join(
            '(SELECT tanggal, pegawai_id, SUM(total_gaji_lembur) AS total_gaji_lembur 
                      FROM abs_lembur 
        ...
        $this->db->join(
            '(SELECT tanggal, pegawai_id, SUM(nilai_tambahan) AS nilai_tambahan 
                      FROM abs_tambahan_lain 
                      WHERE tanggal BETWEEN \'' . $start_date . '\' AND \'' . $end_date . '\'
        ...
        $this->db->join(
            '(SELECT tanggal, pegawai_id, SUM(nilai) AS nilai 
                      FROM abs_potongan 
                      WHERE tanggal BETWEEN \'' . $start_date . '\' AND \'' . $end_date . '\'
        ...
        GROUP BY tanggal, pegawai_id
            ...
            "abs_deposit.pegawai_id = abs_rekap_absensi.pegawai_id AND abs_deposit.tanggal = abs_rekap_absensi.tanggal",
            'left'
        );
        ...
        $this->db->join(
            '(SELECT tanggal, pegawai_id, SUM(nilai) AS total_bayar 
                      FROM abs_kasbon 
